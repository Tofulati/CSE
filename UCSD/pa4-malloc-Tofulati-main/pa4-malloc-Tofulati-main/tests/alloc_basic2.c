...
int main()
{
    ...
    struct block_header *hdr = (struct block_header *)
	 ...
	 printf("Heap start: %p\n", heapstart);  // Check where heap starts (should match expected start)
    printf("Allocated memory pointer: %p\n", ptr);  // Check the actual allocated memory address
    printf("ptr - heapstart: %ld\n", (char *)ptr - (char *)heapstart);  // Debug offset between ptr and heapstart
    printf("Expected offset: 104\n");
